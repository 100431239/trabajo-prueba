...
st.subheader("Participants in descending order by received grants")
...
st.subheader("Project coordinators in ascending order by shortName")
...
if keywords:
    filtered_projects_df = filtered_projects_df[filtered_projects_df["objcetive"].apply(lambda x: any(kw in x for kw in keywords))]
